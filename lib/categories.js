...
const MIN_PER_WEEK_FLOOR = 1; // every workout type requires at least 1x/week — 0 is never allowed
function clampMin(value, fallback) {
  const n = Number(value);
  return Math.max(MIN_PER_WEEK_FLOOR, isNaN(n) ? fallback : n);
}
...
async function upsertCategory(token, email, category) {
  ...
  const record = {
    ...
    min: category.min !== undefined ? clampMin(category.min, existing.min) : existing.min,
